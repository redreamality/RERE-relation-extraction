...
class GPT2_ML_Decoder(GPT2_ML):
	...
	def call(self, inputs, past=None):
		...
		for i in range(self.num_hidden_layers):
			output, present = self.apply_main_layers(output, i, past=past[i])
	...
	# eager, very fast!
	def sample_decode(self, token_ids, topk=5, end_id=511, maxlen=50, minlen=50):
		token_ids = np.array([token_ids])
		outs = self.call(token_ids)
		...
		for ii in range(maxlen):
			...
			outs = self.call(np.array([[r]]), pasts)
			outs, pasts = outs[0], outs[1]
		return rets
	...
	**kwargs
	...
	if config_path is not None:
		if os.path.isdir(config_path):
			...
			config_file = select_one(os.listdir(model_path), lambda x:'config.json' in x)
			checkpoint_file = select_one(os.listdir(model_path), lambda x:'.meta' in x)
			assert config_file is not None
			if checkpoint_file:
				checkpoint_path = os.path.join(model_path, '.'.join(checkpoint_file.split('.')[:-1]))
	...
	models = {
		...
		'gpt2_ml': GPT2_ML,
		'gpt2_ml_decoder': GPT2_ML_Decoder,
		't5': T5,
