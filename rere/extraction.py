...
dname = sys.argv[1]
datadir = '../data/'
trains = utils.LoadJsons(datadir+dname+'/new_train.json')
valids = utils.LoadJsons(datadir+dname+'/new_valid.json')
tests = utils.LoadJsons(datadir+dname+'/new_test.json')
thre_rc=0.2
thre_ee=0.4
try:
...
class RCModel:
    def __init__(self):
        ...
        self.model = tf.keras.models.Model(inputs=self.bert.input, outputs=pos)
...
class EEModel:
    def __init__(self):
        ...
        self.model = tf.keras.models.Model(inputs=self.bert.input, outputs=pos)
        bt.lock_transformer_layers(self.bert, 3)
        self.model_ready = False
...
if __name__ == '__main__':
    ...
    if 'train' in  sys.argv:
        rc.train(trains, batch_size=32, epochs=20)
